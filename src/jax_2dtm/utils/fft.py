...
def nufft(
    density: Array, coords: Array, box: Array, shape: Tuple[int, int, int], eps: float = 1e-6) -> Array:
    """
    ...
    tf_nufft1 = jax2tf.call_tf(_tf_nufft1, output_shape_dtype=jax.ShapeDtypeStruct(shape, masked_density.dtype))
    ft = tf_nufft1(masked_density, periodic_coords, shape, eps)
    # ft = jax_nufft1(shape, masked_density, *periodic_coords.T)#, eps=eps)
...
def jax_nufft1(shape, density, x, y, z, eps=1e-6):
    """
...
def _tf_nufft1(source, points, shape, tol):
    ...
    """
    return tfft.nufft(source, points, grid_shape=shape, transform_type='type_1', tol=tol.numpy())
