...
def test_jit(state, scattering, weights_and_coordinates, resolution, test_image):
    """
    ...
    def build_specimen(voxels):
        density = cs.ElectronGrid(weights=voxels["weights"], coordinates=voxels["coordinates"])
        return cs.Specimen(density=density, resolution=resolution)

    def build_model(voxels):
        specimen = build_specimen(voxels)
        return cs.GaussianImage(specimen=specimen, scattering=scattering, state=state, observed=test_image)
    ...
    np.testing.assert_allclose(compute_image(weights_and_coordinates), unjitted_model.sample())
    np.testing.assert_allclose(compute_loss(weights_and_coordinates), unjitted_model.log_probability())
...
def test_equinox_jit(likelihood_model):
    ...
    np.testing.assert_allclose(compute_image(likelihood_model), likelihood_model.sample())
    np.testing.assert_allclose(compute_loss(likelihood_model), likelihood_model())


def test_equinox_value_and_grad(likelihood_model):

    @eqx.filter_jit
    def update(model, offset_z):
        where = lambda m: m.state.pose.offset_z
        return eqx.tree_at(where, model, offset_z)

    @eqx.filter_value_and_grad
    def compute_loss(offset_z):
        model = update(likelihood_model, offset_z)
        return model()
